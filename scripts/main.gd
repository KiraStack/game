...
const include = ["player", "ui", "systems", "autoload"] # Folders to include in the scan
	...
	while file_name != "":
		if dir.current_is_dir():
			if file_name != "." and file_name != "..":
				...
				if directory == path and file_name not in include:
			...
			if file_name.ends_with(".gd") and file_name != "main.gd":
				...
				if script is GDScript:
					...
					push_warning("Non-GDScript file encountered: " + file_name)
